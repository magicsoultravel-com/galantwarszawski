document.addEventListener('DOMContentLoaded', function() {
    ...
    const modalContent = fileEditorModal.querySelector('.modal-content');
    const closeButton = document.querySelector('.close-button');
    const closeModalButton = document.getElementById('closeModalButton');
    // NOTE: The expandModalButton has been removed from the HTML, so this line is now commented out.
    // const expandModalButton = document.getElementById('expandModalButton'); // OLD
    ...
    // NEW: Elements for the resizable functionality
    const fileEditorResizer = fileEditorModal.querySelector('.resizer-handle');
    const fileEditorModalContent = fileEditorModal.querySelector('.modal-content');
    const fileEditorTextarea = document.getElementById('fileContentEditor');
    ...
    function closeFileEditorModal() {
        ...
        saveFeedback.textContent = '';
        saveFileButton.disabled = false;
        // The expanded class is no longer needed for the modal since it's resizable
        // modalContent.classList.remove('expanded'); 
    }

    // OLD: Function to handle modal expansion (now removed as it's replaced by the resizer)
    // function toggleModalExpansion() {
    //      modalContent.classList.toggle('expanded');
    //      if (modalContent.classList.contains('expanded')) {
    //          expandModalButton.innerHTML = '&larr;&rarr;';
    //      } else {
    //          expandModalButton.innerHTML = '↔';
    //      }
    // }

    // NEW: Functions for the drag-and-resize functionality
    function resizeModal(e) {
        // Calculate new width and height based on mouse position
        const newWidth = Math.min(Math.max(e.clientX - fileEditorModalContent.getBoundingClientRect().left, 300), window.innerWidth * 0.9);
        const newHeight = Math.min(Math.max(e.clientY - fileEditorModalContent.getBoundingClientRect().top, 200), window.innerHeight * 0.8);

        // Apply the new dimensions to the modal and the textarea
        fileEditorModalContent.style.width = newWidth + 'px';
        fileEditorModalContent.style.height = newHeight + 'px';
        
        // This makes sure the textarea fits the resizable content box
        const newTextareaWidth = newWidth - (parseFloat(getComputedStyle(fileEditorModalContent).paddingLeft) + parseFloat(getComputedStyle(fileEditorModalContent).paddingRight));
        const newTextareaHeight = newHeight - (fileEditorTextarea.offsetTop + document.querySelector('.modal-buttons').offsetHeight + parseFloat(getComputedStyle(document.querySelector('.modal-buttons')).marginTop));
        fileEditorTextarea.style.width = newTextareaWidth + 'px';
        fileEditorTextarea.style.height = newTextareaHeight + 'px';
    }
    ...
    // NEW: Add the mousedown event listener to the resizer handle
    if (fileEditorResizer) {
        fileEditorResizer.addEventListener('mousedown', function(e) {
            e.preventDefault();
    ...
    fileBrowserTable.addEventListener('click', function(event) {
        ...
        if (target.closest('.copy-to-clipboard-btn')) {
        ...
        } else if (target.closest('.replace-file-btn')) { // This is the new, missing event handler
            const replaceButton = target.closest('.replace-file-btn');
            const filePath = replaceButton.dataset.filePath;
            if (!filePath) {
                console.error('File path not found for replace action.');
                return;
            }
            ...
            navigator.clipboard.readText()
                .then(clipboardText => {
                    const confirmReplace = confirm(`Are you sure you want to replace the content of "${decodeURIComponent(filePath)}" with content from your clipboard? This cannot be undone.`);
                    if (confirmReplace) {
                        // Create a temporary object to hold the data for the save function
                        const tempFileEditor = {
                            value: clipboardText
                        };
                        // Temporarily assign this value and then call the save logic
                        fileContentEditor.value = clipboardText;
                        currentEditingFilePath = filePath;
                        saveFile().then(() => {
                ...
                .catch(err => {
                    ...
                    alert('Failed to read from clipboard. Please ensure you have granted permission to read from clipboard and try again.');
                });
        }
    });
    ...
    // OLD: expandModalButton listener removed as it's no longer used
    // if (expandModalButton) expandModalButton.addEventListener('click', toggleModalExpansion); 
    ...
    if (copyEditorContentButton) copyEditorContentButton.addEventListener('click', async function() {
        try {
            ...
            // Use saveFeedback for consistency
            saveFeedback.textContent = 'Copied!';
            saveFeedback.style.color = '#27ae60';
            setTimeout(() => { saveFeedback.textContent = ''; }, 1500);
        } catch (err) {
            console.error('Failed to copy content from editor: ', err);
            saveFeedback.textContent = 'Copy failed!';
            saveFeedback.style.color = '#e74c3c';
            setTimeout(() => { saveFeedback.textContent = ''; }, 1500);
        }
    });
    ...
    if (pasteIntoEditorButton) pasteIntoEditorButton.addEventListener('click', async function() {
        try {
            ...
            // Display "Pasted!" feedback using the saveFeedback element
            saveFeedback.textContent = 'Pasted!';
            saveFeedback.style.color = '#27ae60';
            setTimeout(() => { saveFeedback.textContent = ''; }, 1500);

        } catch (err) {
            console.error('Failed to paste content into editor: ', err);
            saveFeedback.textContent = 'Paste failed!';
            saveFeedback.style.color = '#e74c3c';
            setTimeout(() => { saveFeedback.textContent = ''; }, 1500);
        }
    });
